...
const { generateThroughputScript } = require('.')

test('generateThroughputScript', t => {
  t.plan(3)

  const input = [
    [ { throughput: { mean: 123.4, min: 12.3, max: 234.5 } }, { throughput: { mean: 345.6, min: 34.5, max: 456.7 } }, { throughput: { mean: 234.5, min: 23.4, max: 345.6 } } ],
    [ { throughput: { mean: 234.5, min: 23.4, max: 345.6 } }, { throughput: { mean: 456.7, min: 45.6, max: 567.8 } }, { throughput: { mean: 345.6, min: 34.5, max: 456.7 } } ]
  ]

  const { mean, min, max } = generateThroughputScript(input)
