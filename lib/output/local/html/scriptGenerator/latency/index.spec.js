...
const { generateRequestsScript } = require('.')

test('generateRequestsScript', t => {
  t.plan(3)

  const input = [
    [ { requests: { mean: 123.4, min: 12.3, max: 234.5 } }, { requests: { mean: 345.6, min: 34.5, max: 456.7 } }, { requests: { mean: 234.5, min: 23.4, max: 345.6 } } ],
    [ { requests: { mean: 234.5, min: 23.4, max: 345.6 } }, { requests: { mean: 456.7, min: 45.6, max: 567.8 } }, { requests: { mean: 345.6, min: 34.5, max: 456.7 } } ]
  ]

  const { mean, min, max } = generateRequestsScript(input)
