...
const { generateErrorsScript } = require('.')

test('generateErrorsScript', t => {
  t.plan(1)

  const input = [
    [ { errors: 1 }, { errors: 5 }, { errors: 3} ],
    [ { errors: 6 }, { errors: 2 }, { errors: 4 }]
  ]

  const actual = generateErrorsScript(input)
