...
const buildHead = (title) => {
  ...
  const styleElem = `${style}${bassCss}${closeStyle}`

  return `${doctype}${html}${head}${charset}${viewport}${titleElem}${styleElem}${closeHead}`
}

const buildGraphHead = (title) => {
  const opts = { encoding: 'utf8', flag: 'r' }
  ...
  return `${doctype}${html}${head}${charset}${viewport}${titleElem}${styleElem}${scriptElem}${closeHead}${body}`
}

const buildTail = () => {
  return `${closeBody}${closeHtml}`
}
