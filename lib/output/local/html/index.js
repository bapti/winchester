...
const outputHtml = (title, results, config) => {
  ...
  outputTimeouts(title, results, config.timeout, fullOutputPath)
}
...
const outputTimeouts = (title, results, required, fullOutputPath) => {
  if (required) {
    ...
    const timeouts404Html = build404(`${title} - Errors`, 'Timeouts metrics not requested')
    writeHtml(timeouts404Html, fullOutputPath, 'errors')
  }
}
