...
const {head, tail} = require('./constants')
...
function makeOutputPaths(projectRoot) {
  ...
  return {
    outputPath: path.resolve(projectRoot, `output-${ts}`),
...
function outputData(data, opts, dataType, outputPath) {
  ...
  fs.writeFileSync(countPath, countPg)
}

function outputErrors({countData, countOpts}, errorsPath) {
  outputCount(countData, countOpts, 'count', errorsPath)
}
...
function outputTimeouts({countData, countOpts}, timeoutsPath) {
  outputCount(countData, countOpts, 'count', timeoutsPath)
}
