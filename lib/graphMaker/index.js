...
function graph(resultRows) {
  ...
  const mins = requltRows.map(resultRow => resultRow.map(result => result.requests.min))
  const maxs = resultRows.map(resultRow => resultRow.map(result => result.requests.max))
