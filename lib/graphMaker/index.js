...
function makeOutputPaths(outputRoot) {
  const ts = Date.now()
  const outputPath = path.resolve(outputRoot, `output-${ts}`)
...
function graph(resultRows, outputRoot) {
  ...
  const outputPaths = makeOutputPaths(outputRoot)
