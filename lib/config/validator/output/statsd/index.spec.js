...
test('validateStatsDOutput', (t) => {
  ...
  t.throws(() => { validateStatsDOutput(missingGlobalPrefix) }, Error('StatsD globalPrefix must be specified'))

  const invalidGlobalPrefix = { host: 'host', port: 1234, globalPrefix: 122 }
  t.throws(() => { validateStatsDOutput(invalidGlobalPrefix) }, Error('StatsD globalPrefix must be a string'))

  t.same(true, validateStatsDOutput(null))
  t.same(true, validateStatsDOutput(undefined))

  const validStatsD = { host: 'host', port: 1234, globalPrefix: 'globalPrefix' }
