...
const { isBool, isNum, isObj, isStr } = require('../../../utils/is')
...
const validateTarget = (target) => {
  ...
  if (hasBodyFile(targetKeys) && !isStr(target.bodyFile))
    throw new Error('Body File must be a string.')

  if(hasBody(targetKeys) && hasBodyFile(targetKeys) && target.body !== '' && target.bodyFile !== '')
    console.error('Body & Body File specified, contents of Body File will be used')

  if (hasIdReplacement(targetKeys) && !isBool(target.idReplacement))
...
const hasBodyFile = targetKeys => has(targetKeys)('bodyFile')
const hasIdReplacement = targetKeys => has(targetKeys)('idReplacement')
